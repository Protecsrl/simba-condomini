...
namespace Simba.Businness
{
    ...
    public class User : BusinnessBase
    {
        ...
        public Simba.DataLayer.Database.User GetUser()
        {
            using (UnitOfWork uw = new UnitOfWork())
            {
                ...
                var user = uw.Query<Simba.DataLayer.Database.User>().Where(d => d.Username == claim.Value).First();
                return user;
            }
        }

        public Simba.DataLayer.Database.User GetUser(string userid, string password)
        {
            using (UnitOfWork uw = new UnitOfWork())
            {

                var user = uw.Query<Simba.DataLayer.Database.User>().Where(d => d.Username == userid && d.Password == password);
