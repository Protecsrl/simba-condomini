...
//                FileSystemData.BusinessObjects.FileSystemStoreObject
namespace FileSystemData.BusinessObjects
{
    ...
    public class FileSystemStoreObject : BaseObject, IFileData, IEmptyCheckable
    {
        ...
        public string RealFileName
        {
            get
            {
                if (!string.IsNullOrEmpty(FileName) && Oid != Guid.Empty)
                {
                    ...
                    if (!string.IsNullOrEmpty(this.SubPath))
                        return Path.Combine(@"\\g6npaitmi\EAMS-Doc\", SubPath, string.Format("{0}-{1}", Oid, FileName));
                    else
                        return Path.Combine(@"\\g6npaitmi\EAMS-Doc\",  string.Format("{0}-{1}", Oid, FileName));
                }
        ...
        {
            get { return GetPropertyValue<string>("SubPath"); }
            set { SetPropertyValue("SubPath", value); }
        }
