CREATE OR REPLACE PACKAGE PK_DC_SALAOPERATIVA is
  ...
  /*  procedure REPORT_RDL(icodiciRDL in varchar2,
  oMessaggio out varchar2,
  IO_CURSOR  IN OUT T_CURSOR);*/
  procedure REPORT_RDL(icodiciRDL     in varchar2,
                       icodiciRDL_Lob in CLOB,
                       oMessaggio     out varchar2,
                       IO_CURSOR      IN OUT T_CURSOR);

  procedure REPORT_RDL_NEW(icodiciRDL in varchar2,
                           oMessaggio out varchar2,
                           IO_CURSOR  IN OUT T_CURSOR);
  procedure set_apparato_mp_desc(icodiciRDL in varchar2,
                                 oMessaggio out varchar2);
...
create or replace package body PK_DC_SALAOPERATIVA is
        ...
        from risorseteam ri,
            ...
            ,
             ...
             (select regrdl.risorsateam,
                     count(regrdl.rowid) as nrattemergenza
                from regrdl, RDL
  ...
  *\
   ...
   if  v_apparatomp<>nn.apparatomp 
    then
     v_componenti:=nn.descrizione;
      ...
      l_componenti:=length(v_componenti)+ length('-')+length(nn.descrizione);
     
      if (l_componenti>=4000) then 
      v_componenti:=substr(v_componenti,3997)||'..';
      update TAPPARATO_COMPONENTI ac
      --set ac.componenti=ac.componenti||'-'||nn.descrizione
      set ac.componenti=v_componenti
  ...
  OPEN V_CURSOR FOR 
  select 
  substr(nvl(v.componenti,'NA'),3999) as ComponentiManutenzione,
  ...
  t.RICHIEDENTE,
  t.PRIORITA,
  t.PRIORITATIPOINTERVENTO,
  t.REFAMMINISTRATIVO,
  t.CODICEODL,
  t.CODREGRDL,
  t.OIDEDIFICIO,
  t.OIDREFERENTECOFELY,
  t.OIDCATEGORIA,
  t.OIDSMISTAMENTO,
  t.COMPONENTE1,
  t.COMPONENTE2,
  ...
  t.CODSCHEDEMP,
  t.CODSCHEDAMPUNI,
  t.DESCRIZIONEMANUTENZIONE,
  t.FREQUENZADESCRIZIONE,
  t.FREQUENZACOD_DESCRIZIONE,
  t.PASSOSCHEDAMP,
  t.NORDINE,
  nvl(t.INSOURCING,0) as INSOURCING,
  NVL(t.APPARATOMP,0) as APPARATOMP,
  t.APPARATOPADRE,
  t.APPARATOSOSTEGNO,
  t.OIDAPPARATO
  ...
  END insourcing,
      --   a.keyplan,
        ...
        nvl(a.apparatomp,0) as apparatomp,
  ...
  *\
  ...
  (tbl_sql.s_sql,
  tbl_sql.data)
  values('REPORT_RDL input icodiciRDL primo step: '||icodiciRDL,sysdate);
  commit;
  OPEN V_CURSOR FOR
  select
  nvl(t.componenti,'NA') as ComponentiManutenzione,
  t.componente2||t.componente1 as CorpoMP,
  t.componente3 as OrdinePasso,
  t.* from
  (
  ...
  t.oid as CodiceRdL,
         ...
         to_char(t.datacreazione, 'dd/mm/yyyy HH24:MI:SS') as datacreazione,
         to_char(t.datarichiesta, 'dd/mm/yyyy HH24:MI:SS') as datarichiesta,
         nvl(to_char(t.dataupdate, 'dd/mm/yyyy HH24:MI:SS'),'na') as dataaggiornamento,
  ...
  END insourcing,
    ...
    --mansioni ms,
         ...
         ( select mpd.rdl, mp.apparatoschedamp, am.schedamp,
           am.descrizionemanutenzione, am.codschedemp, am.cod_uni,
           smp.passoschedamp, smp.nordine,
             f.cod_descrizione as codfrequenza, f.descrizione as frequenza, 
             am.insourcing
             from mpattivitapianificatedett mpd ,mpattivitapianificate mp,
              apparatoschedamp am, schedemp sm,
              schedemppassi smp, frequenze f
  ...
  (tbl_sql.s_sql,
  tbl_sql.data)
  values('REPORT_RDL input icodiciRDL secondo step: '||icodiciRDL,sysdate);
  commit;
  IO_CURSOR := V_CURSOR;
  ...
  procedure REPORT_RDL(icodiciRDL     in varchar2,
    ...
    v_componenti varchar2(4000);
    l_componenti number := 0;
    v_codici     varchar2(10000);
    l_com  number := 1;
  begin
    --CODICE := '162870,162871,162873,162877,162878,162879,162880,162889,162890,162891,162892,162895,162898,162899,162868,162869,162875,162876,162881,162884,162886,162893,162896,162897,162874,162882,162883,162885,162887,162888,162894,162900,';
    insert into tbl_sql
      (tbl_sql.s_sql, tbl_sql.data)
    values
      ('REPORT_RDL input icodiciRDL: ' || icodiciRDL, sysdate);
    commit;
    insert into tbl_sql
      (tbl_sql.s_sql, tbl_sql.data)
    values
      ('REPORT_RDL input icodiciRDL_Lob: ' || icodiciRDL_Lob, sysdate);
    commit;
    
    select count(*) into l_com
  from (select dbms_lob.substr(icodiciRDL_Lob, 4000, 1) as mio
          from dual);
    
        insert into tbl_sql
      (tbl_sql.s_sql, tbl_sql.data)
    values
      ('REPORT_RDL input conta _Lob: ' || l_com, sysdate);
    commit;
    
    
    select icodiciRDL into v_codici from dual;
    
    
    
    
    OPEN V_CURSOR FOR
      select nvl(v.componenti, 'NA') as ComponentiManutenzione,
             t.componente2 || t.componente1 as CorpoMP,
             t.componente3 as OrdinePasso,
             --t.* 
             t.CODICE,
        ...
        from (select to_Char(t.oid) as Codice,
                     --------
                     t.oid         as CodiceRdL,
                     e.descrizione as Edificio,
                     -----
                     ...
                     CASE t.categoria
                       WHEN 4 THEN
                        ' '
                       WHEN 1 THEN
                        'Scheda Mp:' || rmd.codschedemp ||
                        ' - Cod Scheda Mp UNI:' || rmd.cod_uni ||
                        ' - Frequenza:' || rmd.frequenza
                       WHEN 5 THEN
                        'Scheda Mp:' || rma.codschedemp ||
                        ' - Cod Scheda Mp UNI:' || rma.cod_uni ||
                        ' - Frequenza:' || rma.frequenza
                       ELSE
                        ' '
                     end as componente1,
                     ...
                     CASE t.categoria
                       WHEN 4 THEN
                        'Problema:' || prac.descrizione || ' Causa: ' ||
                        cau.descrizione
                       WHEN 1 THEN
                        ' '
                       WHEN 5 THEN
                     ...
                     CASE t.categoria
                       WHEN 4 THEN
                        ' '
                       WHEN 1 THEN
                        'Nr:' || rmd.nordine || ' - Passo ' ||
                        rmd.passoschedamp
                       WHEN 5 THEN
                        'Nr:' || rma.nordine || ' - Passo ' ||
                        rma.passoschedamp
                       ELSE
                        ' '
                     END componente3,
                     ...
                     CASE t.categoria
                       WHEN 1 THEN
                        rmd.descrizionemanutenzione
                       WHEN 5 THEN
                        rma.descrizionemanutenzione
                       ELSE
                        ' '
                     END descrizionemanutenzione,
                     ...
                     case
                       when a.apparatopadre is not null then
                        (select b.cod_descrizione
                           from apparato b
                          where b.oid = a.apparatopadre)
                       else
                        'NA'
                     
                     end as apparatopadre,
                     case
                       when a.apparatosostegno is not null then
                        (select b.cod_descrizione
                           from apparato b
                          where b.oid = a.apparatosostegno)
                       else
                        'NA'
                     end as APPARATOSOSTEGNO,
                ...
                from RDL    t,
                     ...
                     (select rsa.rdl,
                             rsa.apparatoschedamp,
                             am.schedamp,
                             am.descrizionemanutenzione,
                             am.codschedemp,
                             am.cod_uni,
                             smp.passoschedamp,
                             smp.nordine,
                             f.cod_descrizione          as codfrequenza,
                             f.descrizione              as frequenza,
                             am.insourcing
                        from rdlapparatoschedemp rsa,
                             apparatoschedamp    am,
                             schedemp            sm,
                             schedemppassi       smp,
                             frequenze           f
                       where am.oid = rsa.apparatoschedamp
                         and am.schedamp = sm.oid
                         and f.oid = am.frequenzaopt
                         and smp.schedemp = sm.oid) rma,
                     ...
                     (select mpd.rdl,
                             ...
                             f.descrizione              as frequenza,
                             am.insourcing
                        from mpattivitapianificatedett mpd,
                             mpattivitapianificate     mp,
                             apparatoschedamp          am,
                             schedemp                  sm,
                             schedemppassi             smp,
                             frequenze                 f
                       where mpd.mpattivitapianificate = mp.oid
                         and am.oid = mp.apparatoschedamp
                         and am.schedamp = sm.oid
                         and f.oid = am.frequenzaopt
                         and smp.schedemp = sm.oid) rmd
               ...
               where e.oid = t.edificio
                 ...
                 and t.regrdl = rr.oid
                    --and ms.oid = rt.RisorsaMansione
                 and to_Char(t.oid) in
                     (select regexp_substr(v_codici, '[^,]+', 1, rownum) str
                        from dual
                      connect by level <=
                                 regexp_count(v_codici, '[^,]+'))) t,
             --  TAPPARATO_COMPONENTI_ v
             (SELECT V.APPARATOMP,
                     count(v.oid) members,
                     substr(XMLCast(XMLAGG(XMLELEMENT(e,
                                                      ',' || v.descrizione)) as CLOB),
                            2) AS COMPONENTI
                FROM apparato V
               WHERE V.Apparatomp IS NOT NULL
               GROUP BY V.Apparatomp) v
       where v.apparatomp(+) = t.oidapparato;
    IO_CURSOR := V_CURSOR;
    ...
    */
    ...
    OPEN V_CURSOR FOR
      ...
      --nvl(v.componenti,'NA') as ComponentiManutenzione,
       ...
       (SELECT
        --x.oidapparatomp,
         rtrim(xmlagg(XMLELEMENT(e, x.descrizione, ';').EXTRACT('//text()'))
               .GetClobVal(),
               ',') as componenti
          from (select b.oid as oidapparatomp, a.descrizione
                  from apparato a, apparato b
                 where b.oid = a.apparatomp) x
         where x.oidapparatomp = t.oidapparato
         group by x.oidapparatomp) as ComponentiManutenzione,
        ...
        from (select to_Char(t.oid) as Codice,
               ...
               where e.oid = t.edificio
                 ...
                 and to_Char(t.oid) in
                     (select regexp_substr(icodiciRDL, '[^,]+', 1, rownum) str
                        from dual
                      connect by level <= regexp_count(icodiciRDL, '[^,]+'))) t;
    IO_CURSOR := V_CURSOR;
  ...
  begin
    ...
    for nn in (select a.apparatomp, a.oid, a.descrizione
                 from REGRDL t, apparato a
                where t.apparato = a.oid
                  and a.apparatomp is not null
                group by a.apparatomp, a.oid, a.descrizione
                order by a.apparatomp, a.descrizione
               /*    in (
               select regexp_substr (icodiciRDL, '[^,]+',1, rownum) str 
               from dual 
               connect by level <= regexp_count (icodiciRDL, '[^,]+')  ) )*/
      ...
      if v_apparatomp <> nn.apparatomp then
        v_componenti := '';
        v_componenti := nn.descrizione;
        insert into TAPPARATO_COMPONENTI_
          (apparatomp, componenti)
        values
          (nn.apparatomp, v_componenti);
        commit;
      else
        l_componenti := length(v_componenti) + length('-') +
                        length(nn.descrizione);
        if (l_componenti >= 4000) then
          exit;
        end if;
        v_componenti := v_componenti || '-' || nn.descrizione;
        update TAPPARATO_COMPONENTI_ ac
           set ac.componenti = v_componenti
         where ac.apparatomp = nn.apparatomp;
        commit;
      end if;
