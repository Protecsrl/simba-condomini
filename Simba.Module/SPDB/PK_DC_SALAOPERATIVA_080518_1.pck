CREATE OR REPLACE PACKAGE PK_DC_SALAOPERATIVA is
  ...
  procedure CALCOLO_COMPONENTE(icodiciRDL in varchar2,
                               oMessaggio out varchar2,
                               IO_CURSOR  IN OUT T_CURSOR);
...
create or replace package body PK_DC_SALAOPERATIVA is
  ...
  begin
    ...
    */
    ...
    for nn in (select a.descrizione, a.apparatomp
                 from apparato a
                where a.apparatomp in
                         ...
                         from (select to_Char(t.oid) as Codice,
                                 ...
                                 from RDL    t,
                                      ...
                                      (select mpd.rdl,
                                         ...
                                         from mpattivitapianificatedett mpd,
                                              ...
                                              schedemppassi             smp,
                                              frequenze                 f
                                        where mpd.mpattivitapianificate =
                                              mp.oid
                                ...
                                where e.oid = t.edificio
                                  ...
                                  and to_Char(t.oid) in
                                      (select regexp_substr(icodiciRDL,
                                                            '[^,]+',
                                                            1,
                                                            rownum) str
                                         from dual
                                       connect by level <=
                                                  regexp_count(icodiciRDL,
                                                               '[^,]+')))
                              
                              t,
                              apparato a
                        where a.descrizione = t.Apparato
                          and t.Codice in
                              (select regexp_substr(icodiciRDL,
                                                    '[^,]+',
                                                    1,
    ...
    OPEN V_CURSOR FOR
     /* select nvl(v.componenti, 'NA') as ComponentiManutenzione,
             ...
             t.*
        from (*/
        select to_Char(t.oid) as Codice,
        ---
...
FREQUENZACOD_DESCRIZIONE
PASSOSCHEDAMP
NORDINE, INSOURCING
*/

         ' ' as ComponentiManutenzione,
              ' ' as CorpoMP,
             ' ' as OrdinePasso,
                     --------
                     t.oid         as CodiceRdL,
                     ...
                     NVL(t.odl, 0) as CodiceOdL,
                      NVL( t.regrdl, 0) as CodRegRdL,
                     
                     e.oid              as OidEdificio,
                      NVL(cm.referentecofely , 0) as oidreferentecofely,
                     ...
                     CASE nvl(t.categoria,0)
                       ...
                       ELSE
                        ' '
                     END CodSchedaMPUNI,--CODSCHEDAMPUNI
                     
                     CASE nvl(t.categoria,0)
                       WHEN 1 THEN
                       nvl(   rmd.descrizionemanutenzione,' ')
                       WHEN 5 THEN
                        nvl(  rma.descrizionemanutenzione,' ')
                       ELSE
                        ' '
                     ...
                     CASE nvl(t.categoria,0)
                       WHEN 1 THEN
                        nvl(rmd.codfrequenza,' ')
                       WHEN 5 THEN
                        nvl(rma.codfrequenza,' ')
                       ELSE
                        ' '
                     ...
                     CASE nvl(t.categoria,0)
                       WHEN 1 THEN
                       nvl(  rmd.passoschedamp,' ')
                       WHEN 5 THEN
                      nvl(   rma.passoschedamp,' ')
                       ELSE
                        ' '
                     END passoschedamp,
...
/*                      ) t,
  ...
  begin
    ...
    OPEN V_CURSOR FOR
      select v.componenti, v.apparatomp from TAPPARATO_COMPONENTI v;
    IO_CURSOR := V_CURSOR;
