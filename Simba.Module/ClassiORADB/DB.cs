...
namespace CAMS.Module.ClassiORADB
{
    public class DB : IDisposable
    {
        ...
        public FileData getFileFromViewId(string viewId)
        {
            ...
            {
                //using (OracleConnection OrclConn = new OracleConnection(CAMS.Module.Classi.SetVarSessione.OracleConnString))
                using (OracleConnection OrclConn = new OracleConnection(iConnessione))
                {
                    OracleCommand command = OrclConn.CreateCommand();
        ...
        public string SetCambioPassword(string UserNameCorrente, string OldPassword, DateTime DataConferma, int Flag)
        {
            ...
            try
            {
                using (var OrclConn = new OracleConnection(iConnessione))
                {
                    //var cmd = new SqlCommand("pk_acount.SetCambioPassword", OrclConn);
                    //cmd.CommandType = CommandType.StoredProcedure;
                    var command = OrclConn.CreateCommand();
                    command.CommandText = "PK_ACCOUNT.SetCambioPassword";//  poi
                    command.CommandType = CommandType.StoredProcedure;

                    var pp = new OracleParameter("iUtente", OracleType.VarChar, 200) { Value = UserNameCorrente };
                    command.Parameters.Add(pp);
                    pp = null;
                    pp = new OracleParameter("iOldPassword", OracleType.VarChar, 200) { Value = OldPassword };
                    command.Parameters.Add(pp);
                    pp = null;
                    pp = new OracleParameter("iDataUpdate", OracleType.DateTime) { Value = DataConferma };
                    command.Parameters.Add(pp);
                    pp = null;
                    pp = new OracleParameter("iFlag", OracleType.Number) { Value = Flag };
                    command.Parameters.Add(pp);
                    pp = null;
