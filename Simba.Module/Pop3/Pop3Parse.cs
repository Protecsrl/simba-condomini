...
namespace Pop3
{
	...
	public class Pop3Parse
	{
        ...
        public static string Received(string line)
        {
            return
                Regex.Replace(line
                , @"^Received: .*[ |<]([a-z|A-Z|0-9|\.|\-|_]+@[a-z|A-Z|0-9|\.|\-|_]+).*$"
                , "$1");
        }
        public static string Message(string line)
        {
            return
                Regex.Replace(line
                , @"^Message-ID: .*[ |<]([a-z|A-Z|0-9|\.|\-|_]+@[a-z|A-Z|0-9|\.|\-|_]+).*$"
                , "$1");
        }

        public static string ID(string line)
        {
            string temp = Regex.Replace(line, @"^\tid .*[ |<]([a-z|A-Z|0-9|\.|\-|_]+@[a-z|A-Z|0-9|\.|\-|_]+).*$", "$1");
            int ind = temp.IndexOf(";");
                string temp1 = temp.Substring(3,ind-3);
            return         temp1;
        }

        public static string Date(string line)
        {
            return Regex.Replace(line, @"Date: ", "");

            //return
            //    Regex.Replace(line
            //    , @"^Date:.*[ |<]([a-z|A-Z|0-9|\.|\-|_]+@[a-z|A-Z|0-9|\.|\-|_]+).*$"
            //    , "$1");
        }
