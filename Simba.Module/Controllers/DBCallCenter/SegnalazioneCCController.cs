...
using CAMS.Module.DBCallCenter;
...
namespace CAMS.Module.Controllers.DBCallCenter
{
    ...
    public partial class SegnalazioneCCController : ViewController
    {
        public SegnalazioneCCController()
        {
            InitializeComponent();
        ...
        private void CreaRdL_Execute(object sender, SimpleActionExecuteEventArgs e)
        {

            IObjectSpace xpObjectSpace = Application.CreateObjectSpace();
            if (xpObjectSpace != null)
            {
                Immobile tEdificio = xpObjectSpace.GetObject<Immobile>(((SegnalazioneCC)View.CurrentObject).Immobile);
                ...
                NuovoRdL.SetMemberValue("Servizio", tEdificio.Impianti.FirstOrDefault());

                Asset _Asset = session.Query<Asset>().Where(w => w.Servizio == NuovoRdL.Servizio).Select(s => s).First();

                NuovoRdL.SetMemberValue("Apparato", _Asset);
        ...
        void ex_ObjectSpace_Committed(ref IObjectSpace xpObjectSpaceRRdl, ref RdL rdl)
        {
            ...
            if (rdl != null)
            {
                ...
                if (isGranted && rdl.Immobile.Contratti.AttivaAutoAssegnazioneTeam && rdl.UltimoStatoSmistamento.Oid == 1)//  solo se è appena creata
                {
                    ...
                    int OidServizio = rdl.Servizio.Oid;
                    int OidCategoria = rdl.Categoria.Oid;
                    ...
                    var query = RegoleQuery.Where(w => w.Immobile.Oid == OidEdificio && w.Servizio.Oid == OidServizio)
        ...
        private void pupWinEdificioCC_Execute(object sender, PopupWindowShowActionExecuteEventArgs e)
        {
            ...
            if (xpObjectSpace != null)
            {
                ...
                if (View is DetailView)
                {
                    ...
                    SegnalazioneCC SegnalazioneCC = (SegnalazioneCC)dv.CurrentObject;

                    Immobile ObjCurr = (Immobile)(((e.PopupWindow)).View).SelectedObjects[0];

                    if (ObjCurr != SegnalazioneCC.Immobile)
                    {
                        SegnalazioneCC.SetMemberValue("Immobile", ObjCurr);
                        //if (ObjCurr.Impianti.Count == 1)
                        //{
                        //    rdl.SetMemberValue("Impianto", ObjCurr.Impianti[0]);
                        //    if (ObjCurr.Impianti[0].APPARATOes.Count == 1)
                        //        rdl.SetMemberValue("Apparato", ObjCurr.Impianti[0].APPARATOes[0]);
                        //}
                        //else
                        //{
        ...
        private void pupWinEdificioCC_CustomizePopupWindowParams(object sender, CustomizePopupWindowParamsEventArgs e)
        {
            IObjectSpace xpObjectSpace = View is ListView ? Application.CreateObjectSpace() : View.ObjectSpace;
            if (xpObjectSpace != null)
            {
                ...
                {
                    DetailView dv = (DetailView)View;
                    SegnalazioneCC rdl = (SegnalazioneCC)dv.CurrentObject;
                    ...
                    //ListRisorseTeamLookUp.Criteria["Criterio"] = CriteriaOperator.Parse(ParCriteria);
                    ...
                    //GroupOperator criteriaObjectAnd = new GroupOperator(GroupOperatorType.And);
                    ...
                    if (!string.IsNullOrEmpty(rdl.RicercaEdificio))
                    { //  string add = string.Format("Contains(Upper({0}),'{1}')", searchCriteria[i], paramValue);
                        ...
                        { 
                        Filtro = string.Format("Contains([Descrizione],'{0}') Or Contains([Indirizzo.FullName],'{0}')", rdl.RicercaEdificio.ToString());
                        criteriaOP2.Operands.Add(CriteriaOperator.Parse(Filtro));
                        Filtro = string.Format("Contains([Indirizzo.Title],'{0}') Or Contains([Indirizzo.Strada],'{0}')", rdl.RicercaEdificio.ToString());
                        criteriaOP2.Operands.Add(CriteriaOperator.Parse(Filtro));
                        Filtro = string.Format("Contains([Indirizzo.Comuni.DenomIta],'{0}') Or Contains([Indirizzo.Provincia.Descrizione],'{0}')", rdl.RicercaEdificio.ToString());
                        criteriaOP2.Operands.Add(CriteriaOperator.Parse(Filtro));
                        Filtro = string.Format("Contains([Indirizzo.Cap],'{0}') Or Contains([Commesse.CentroCosto],'{0}')", rdl.RicercaEdificio.ToString());
                        criteriaOP2.Operands.Add(CriteriaOperator.Parse(Filtro));
                        Filtro = string.Format("Contains([Commesse.WBS],'{0}') Or Contains([Commesse.Cliente.Descrizione],'{0}')", rdl.RicercaEdificio.ToString());
                        criteriaOP2.Operands.Add(CriteriaOperator.Parse(Filtro));
                        Filtro = string.Format("Contains([Commesse.ClienteReferenteContratto.Telefono],'{0}') Or Contains([Commesse.ClienteReferenteContratto.Denominazione],'{0}')", rdl.RicercaEdificio.ToString());
                        criteriaOP2.Operands.Add(CriteriaOperator.Parse(Filtro));
                        Filtro = string.Format("Contains([Commesse.ClienteReferenteContratto.Indirizzo],'{0}') Or Contains([Commesse.ClienteReferenteContratto.Mail],'{0}')", rdl.RicercaEdificio.ToString());
                        criteriaOP2.Operands.Add(CriteriaOperator.Parse(Filtro));
                        }
                        else
                        { 
                        Filtro = string.Format("Contains(Upper([Descrizione]),'{0}') Or Contains(Upper([Indirizzo.FullName]),'{0}')", rdl.RicercaEdificio.ToUpper());
                        criteriaOP2.Operands.Add(CriteriaOperator.Parse(Filtro));
                        Filtro = string.Format("Contains(Upper([Indirizzo.Title]),'{0}') Or Contains(Upper([Indirizzo.Strada]),'{0}')", rdl.RicercaEdificio.ToUpper());
                        criteriaOP2.Operands.Add(CriteriaOperator.Parse(Filtro));
                        Filtro = string.Format("Contains(Upper([Indirizzo.Comuni.DenomIta]),'{0}') Or Contains(Upper([Indirizzo.Provincia.Descrizione]),'{0}')", rdl.RicercaEdificio.ToUpper());
                        criteriaOP2.Operands.Add(CriteriaOperator.Parse(Filtro));
                        Filtro = string.Format("Contains(Upper([Indirizzo.Cap]),'{0}') Or Contains(Upper([Commesse.CentroCosto]),'{0}')", rdl.RicercaEdificio.ToUpper());
                        criteriaOP2.Operands.Add(CriteriaOperator.Parse(Filtro));
                        Filtro = string.Format("Contains(Upper([Commesse.WBS]),'{0}') Or Contains(Upper([Commesse.Cliente.Descrizione]),'{0}')", rdl.RicercaEdificio.ToUpper());
                        criteriaOP2.Operands.Add(CriteriaOperator.Parse(Filtro));
                        Filtro = string.Format("Contains(Upper([Commesse.ClienteReferenteContratto.Telefono]),'{0}') Or Contains(Upper([Commesse.ClienteReferenteContratto.Denominazione]),'{0}')", rdl.RicercaEdificio.ToUpper());
                        criteriaOP2.Operands.Add(CriteriaOperator.Parse(Filtro));
                        }
        ...
        private void acDelEdificioCC_Execute(object sender, SimpleActionExecuteEventArgs e)
        {
            if (View is DetailView)
            {
                DetailView dv = (DetailView)View;
        ...
        private void pupWinNewRichiedenteRdLCC_CustomizePopupWindowParams(object sender, CustomizePopupWindowParamsEventArgs e)
        {
            IObjectSpace xpObjectSpace = Application.CreateObjectSpace();
            SegnalazioneCC rdl = ((DetailView)View).CurrentObject as SegnalazioneCC;
            if (View.Id.Contains("RdL_DetailView"))
            {
                ...
                var view = Application.CreateDetailView(View.ObjectSpace, "Richiedente_DetailView", false, Richiedente);
        ...
        private void pupWinNewRichiedenteRdLCC_Execute(object sender, PopupWindowShowActionExecuteEventArgs e)
        {
            SegnalazioneCC rdl = ((DetailView)View).CurrentObject as SegnalazioneCC;
        ...
        private void pupWinSearchRichiedenteCC_Execute(object sender, PopupWindowShowActionExecuteEventArgs e)
        {
            SegnalazioneCC rdl = ((DetailView)View).CurrentObject as SegnalazioneCC;
            Richiedente Richiedente = ((Richiedente)(((e.PopupWindow)).View).SelectedObjects[0]);
            rdl.SetMemberValue("Richiedente", Richiedente);
        ...
        private void pupWinSearchRichiedenteCC_CustomizePopupWindowParams(object sender, CustomizePopupWindowParamsEventArgs e)
        {
            IObjectSpace xpObjectSpace = Application.CreateObjectSpace();
            SegnalazioneCC rdl = ((DetailView)View).CurrentObject as SegnalazioneCC;
            if (View.Id.Contains("RdL_DetailView"))
